...
var router=function(){
    ...
    myself.selectedBoard = _.find(myself.boards, function(board) { return board.id == boardId })
      ...
      return option;
    }).object().value(), function(board) { React.renderComponent(Board({board: board}), $("#view").get(0)) });
	} else {
...
var listBoards=function(){
  ...
  React.renderComponent(Organizations({organizations: myself.orgBoards}), $("#view").get(0));
};
...
var getBoard=function(board, options, callback){
  console.log(options);
  $("#view").empty();
  $("#view").html("<h1>Loading ...</h1>");
  Trello.get("/boards/"+board,{cards:"open",lists:"open",checklists:"all",members:"all",actions:"all"},function(board){
